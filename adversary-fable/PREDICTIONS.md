...
* **§6 "The runtime estimate for 2x5 (implied by section 3) was wrong by more than 10x"** is mis-cited (§3 has no runtime estimates; §4 has them, 0.5–2 h) and unsupported (the 2x5 Rust run hit its 7,200 s cap incomplete and predates the `walked` key). Measured on the current binary (`--tlimit 90 --threads 2`, contended): 386 pairs / 84.5 s CPU = 4.57 pairs/s over the product-order prefix, which §4 itself identifies as the expensive region → ≤ ~41 CPU-hours for 672,719 pairs, an upper bound; the science lens's rate on later pairs gives ~12 CPU-h. Against §4's 2 h upper end the overrun is between ~6x and ~20x by CPU-hours, or ~2.5x by wall on 8 threads at the upper bound — not a clean "more than 10x". Withdrawn as stated.
