...
## 7. Errata (appended 2026-09-06 evening; pre-registered text above is unchanged)

Found by the record and science lenses of the r5 sweep (`rust/refute/record/r5/FINDINGS.md`, `rust/refute/science/r5/FINDINGS.md`) and re-measured before being written here.

* **§2.2 lists the five cycles8 presence patterns in the wrong product order.** Recomputed from `cycles8.SLOTS`/`cycles8.build` in true `itertools.product` order (`record/r5/pattern_order.py`): index 0 `(0,1,1,1,1,1,1)` [4,4] 46,656 tuples; 1 `(1,0,0,1,1,1,1)` [4,4] 7,776; 2 `(1,0,1,1,1,0,0)` [8] 1,296; 3 `(1,1,0,0,0,1,1)` [8] 1,296; 4 `(1,1,1,0,0,0,0)` [4,4] 216. The §3 pairs list (1,557,900 / 259,650 / 40,830 / 40,830 / 6,210) IS in product order and disagrees with §2.2's label-tuple list "216 + 7,776 + 46,656 + 1,296 + 1,296" in the same cell. The aggregates (54,648 tuples and 1,823,760 pairs for the two-4-cycle patterns = indices 0, 1, 4) are correct. `kakeya-search cycles8 --patterns i,j` selects by the true indices above.
* **§6 "The runtime estimate for 2x5 (implied by section 3) was wrong by more than 10x"** — the citation is a typo (§3 has no runtime estimates; the 0.5–2 h figure is in §4) and the ">10x" was written without a completed run behind it. **The ">10x" itself is RIGHT and is kept.** [This bullet was rewritten 2026-09-06 ~23:45: its first version, written from the record lens's "~2.5x" measurement, WITHDREW the claim — and adjudication FINDING-10 refuted that withdrawal: the 2.5x compared wall-on-8-threads against §4's SEQUENTIAL estimate (a category error), the "prefix probe is an upper bound" premise is backwards (per-pair cost rises ~2.7x across the first 14% of a complete 2x4, so a prefix probe is a LOWER bound), and `rust/run_targets.sh:5` sets T=8, so the recorded incomplete 2x5 run at 7,207 s had already burned **16.0 thread-hours** on disk, which excludes any estimate below that.] Measured: a complete 2x5 costs about **25–40 CPU-hours** (floor 16.0 thread-hours), i.e. 12–73x over §4's 0.5–2 h in §4's own sequential currency, or ~2x by wall at 8 threads. The science lens's ~12 CPU-h figure is excluded by the same floor.
* **§6's 8-cycle cross-check has completed.** `ks8run cycles8 --pool 6 --patterns 8cycle --threads 10`: 19:02:26Z → 20:46:00Z, exit 0, `tested: 81660`, 0 hits, best null. Rows 3 of §0 is now confirmed by two independent routes (this run has no Lemma C prune).
* **§2.2/§6 never mention `t = 3`.** The driver (`cycles8.py:62`) loops `t in (0, 1, 2)`; at POOL6 / 67/40 the `t = 3, r = 0` phase scores 8/5 = 1.600 ≤ 67/40 and was never searched. "Exhausts cycles8 POOL6 under the stated budgets" above was therefore true only for t ≤ 2. Closed 2026-09-06 in pure Python (`rust/refute/science/r5/t3_check.py`): patterns 2,3 → 87,600 (graph, T0) pairs, 0 hits; patterns 1,4 → 236,400 pairs, 0 hits; pattern 0 → 1,368,000 pairs, 0 hits (`t3_idx0_purepy.txt`, 567 s). Total 1,692,000 pairs at t = 3, 0 hits, all five patterns by computation. `t ≥ 4` is budget-infeasible.
* **§2.2's imported 4-cycle floor** was re-measured without a budget in the way (`science/r5/lemmas_check.py` part B): it holds, and the unbudgeted form is what closes t = 3 for the two-4-cycle patterns.
